...
usb_entry_supported() {
    ...
    case "$label" in
        'efi usb device'|'uefi usb device') return 0 ;;
        *) return 1 ;;
    esac
}
...
discover_firmware_entries() {
    ...
    while IFS= read -r line; do
        ...
        if [[ $label == "$RECOVERY_LABEL" || $label == "$LIMINE_LABEL" ]] \
            || foreign_entry_supported "$label" "$line" \
            || usb_entry_supported "$label"; then
        ...
        if foreign_entry_supported "$label" "$line"; then
            FOREIGN_LABELS+=("$label")
        elif usb_entry_supported "$label"; then
            USB_LABELS+=("$label")
        fi
    done <<< "$output"
...
write_config() {
    local target=$1 firmware=$2 staged file initrd label timeout=3

    # Always render the menu. Give multi-boot systems a little longer to make
    # an OS choice, while keeping the common Linux-only path brisk. A generic
    # USB firmware record is persistent on many machines, so it does not by
    # itself turn a Linux-only system into a long-timeout setup.
    (( ${#FOREIGN_LABELS[@]} > 0 )) && timeout=5
