...
hyprarch/
  defaults/          # Universal configs, deployed to ~/.local/share/hyprarch/
    ...
    themes/          # Bundled themes: palette, lock treatment, wallpapers per mode
...
6. Runs `theme-apply.sh` as the user, rendering `dunstrc`, `fuzzel.ini`,
   `ghostty/config`, `hyprlock-theme.conf`, and `~/.config/hyprarch/theme.lua`
   from their `.tmpl` sources and writing `~/.config/hyprarch/theme.json`
...
~/.config/hypr/hyprland.lua         # Generated by install.sh, requires:
  ...
  │    └─ ~/.config/hyprarch/theme.lua  # Window borders from the active theme,
  ...
  └─ ~/.config/hyprarch/monitors.lua  # User monitor arrangement, generated by
...
`~/.config/hyprarch/monitors.json` (source of truth) and of the
`~/.config/hyprarch/monitors.lua` rules generated from it. Entries are keyed by
...
- `wallpaper` (relative to the theme directory), `ghostty` (a Ghostty theme
  name), `iconTheme`, and `label` (the mode's display name).

`defaults/shell/theme-apply.sh` is the only writer of
`~/.config/hyprarch/theme.json` — the persisted `{theme, mode}` selection
...
`~/.local/share/hyprarch/`, plus `~/.config/hyprarch/theme.lua`, which
`hyprland.lua` loads through `pcall` for the window border colours with a
built-in fallback. Templates use `{{path}}` placeholders resolved against the
resolved theme (`{{colors.accent}}`, `{{lock.contrast}}`,
`{{modes.light.ghostty}}`). Substitution is single-pass, an unknown
placeholder or a missing token aborts the render before any output is
touched, and outputs are staged and renamed so a reader never sees a
half-written file. The script then sets the portal colour scheme through
`gsettings`, writes the GTK 3/4 `settings.ini`, sets the mode's wallpaper,
touches the config symlinks for inotify readers, reloads Dunst in place, and
runs `hyprctl reload`. Those side effects are guarded by `command -v`,
...
`Theme.qml` reads `~/.config/hyprarch/theme.json` through a watched
...
translucency, and animations are deliberately not part of a theme; they are
the rice's identity and stay in `Theme.qml` and `hyprland.lua`.
...
- Colour notation differs per consumer and is handled by the renderer's
  ...
  `#rrggbb`). Hyprlock needs `##` before a Pango hex colour, so that template
