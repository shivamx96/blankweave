...
`plymouth/hyprarch/hyprarch.script` from its template next to a copy of the
artwork under `~/.local/share/hyprarch/`, and `status` reports
...
Papirus variant, the staged splash into `/usr/share/plymouth/themes/hyprarch`
...
apply, `hyprarch theme set` runs it through sudo, and `hyprarch theme sync`
...
`hyprarch theme list|status|set <id>|mode dark|light|toggle|sync` is the
